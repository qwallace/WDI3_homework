...
//Write a function called ```whatDoesYourStartupDo``` that displays the message in the console "So....your startup is 
//basically like " + randomThis() + " for " + randomThat() + "?"
...
function randomThis() {
  x =  Math.random();
  y = Math.floor(thisArray.length *x);
  return thisArray[y]
}
function randomThat() {
  x =  Math.random();
  y = Math.floor(thatArray.length *x);
  return thisArray[y]
}
